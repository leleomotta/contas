...
class Receita extends Model
{
    ...
    public function filter($categoria, $conta, $texto, $start_date, $end_date)
    {
        ...
        if ($start_date != '0001-01-01') {
            $filtros = $filtros->whereBetween('receita.Data', [$start_date, $end_date]);
        }
    ...
    public function receitas($start_date, $end_date, $conta)
    {
        ...
        if (!is_null($start_date)) {
            $receitas->where('receita.Data', '>=', $start_date);
        }

        if (!is_null($end_date)) {
            $receitas->where('receita.Data', '<=', $end_date);
        }
    ...
    public function receitasPendente($categoria, $conta, $texto, $start_date, $end_date)
    {
        $retorno = DB::table('receita')
            ->where('receita.Efetivada', '0');

        if ($start_date != '0001-01-01') {
            $retorno = $retorno->whereBetween('receita.Data', [$start_date, $end_date]);
        }
